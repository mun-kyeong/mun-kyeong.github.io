...
다만, 아쉬웠던 점으로는 PR 리뷰를 내가 많이 진행하지 못했던 점과 **처음보는 JUnit test를 사용해보지 않고 바로 제출**해버린 점이다. 도전을 망설이지 말아야 한다고 늘 생각하지만, 새로운 환경에 적응하려고 마음먹는데 까지는 조금은 시간이 걸리는 것 같다. 미션지에서도 JUnit test를 활용해보라고 소개를 해주었지만 PR을 마무리하고 다른 사람들이 활용한 JUnit 코드를 보고 나서야 JUnit test를 소개해준 의도를 이해했던 것 같다.
...
2. 구체적인 기능명세
   ...
   - 사실 기능정의는 사람마다 다르겠지만 나의 경우 `입력 > 기능 > 출력` 이렇게 세 단계로 나누어 기능명세를 작성했다. 커밋 컨벤션으로부터 가져온 아이디어긴 하지만, 각 기능명세 앞에 **태그**를 붙여 기능을 구분했던 시도가 좋았던 것 같다.
...
1. 메서드가 한 기능을 하는지에 대한 확인하는 기준 세우기
   -
